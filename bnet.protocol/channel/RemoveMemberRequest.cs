...
namespace bnet.protocol.channel
{
	public class RemoveMemberRequest : IProtoBuf
	{
		...
		{
			return RemoveMemberRequest.Deserialize(stream, instance, (long)-1);
		}
		...
		public static RemoveMemberRequest DeserializeLengthDelimited(Stream stream, RemoveMemberRequest instance)
		{
			...
			return RemoveMemberRequest.Deserialize(stream, instance, position);
		}
		...
		public override bool Equals(object obj)
		{
			...
			if (this.HasAgentId != removeMemberRequest.HasAgentId || this.HasAgentId && !this.AgentId.Equals(removeMemberRequest.AgentId))
			{
				return false;
			}
			if (!this.MemberId.Equals(removeMemberRequest.MemberId))
			{
				return false;
			}
			if (this.HasReason == removeMemberRequest.HasReason && (!this.HasReason || this.Reason.Equals(removeMemberRequest.Reason)))
			{
				return true;
			}
			return false;
