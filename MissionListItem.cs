...
public class MissionListItem : MonoBehaviour
{
	...
	public void Init(int missionRecID)
	{
		...
		JamGarrisonMobileMission item = (JamGarrisonMobileMission)PersistentMissionData.missionDictionary[this.garrMissionID];
		...
		long num1 = (long)this.missionDurationInSeconds - num;
		num1 = (num1 <= (long)0 ? (long)0 : num1);
		bool flag = (item.MissionState != 1 ? false : num1 > (long)0);
		...
		MissionRewardDisplay.InitMissionRewards(this.missionRewardDisplayPrefab, this.missionRewardGroup.transform, item.Reward);
	...
	private void Update()
	{
		...
		long num1 = (long)this.missionDurationInSeconds - num;
		num1 = (num1 <= (long)0 ? (long)0 : num1);
		Duration duration = new Duration((int)num1, false);
		this.missionTimeRemainingText.text = string.Concat(duration.DurationString, " <color=#ff0000ff>(In Progress)</color>");
	}
}