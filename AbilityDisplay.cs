...
public class AbilityDisplay : MonoBehaviour
{
	...
	public void SetAbility(int garrAbilityID, bool hideCounterInfo = false, bool hideName = false, FollowerDetailView followerDetailView = null)
	{
		...
		GarrAbilityRec garrAbilityRec = StaticDB.garrAbilityDB.GetRecord(this.m_garrAbilityID);
		if (garrAbilityRec == null)
		{
			Debug.LogWarning(string.Concat("Invalid garrAbilityID ", this.m_garrAbilityID));
			return;
		}
		this.m_abilityNameText.text = garrAbilityRec.Name;
		if (garrAbilityRec.IconFileDataID <= 0)
		{
			this.m_abilityIcon.enabled = false;
		}
		else
		{
			Sprite sprite1 = GeneralHelpers.LoadIconAsset(AssetBundleType.Icons, garrAbilityRec.IconFileDataID);
			if (sprite1 != null)
			{
				this.m_abilityIcon.sprite = sprite1;
			...
			else if (this.m_iconErrorText != null)
			{
				...
				this.m_iconErrorText.text = string.Concat(string.Empty, garrAbilityRec.IconFileDataID);
			}
			this.m_abilityIcon.enabled = true;
		}
		this.m_garrAbilityID = garrAbilityRec.ID;
		GarrAbilityCategoryRec garrAbilityCategoryRec = StaticDB.garrAbilityCategoryDB.GetRecord((int)garrAbilityRec.GarrAbilityCategoryID);
		...
		if (this.m_counteredMechanicGroup != null)
		{
			if (!hideCounterInfo)
			{
				...
				StaticDB.garrAbilityEffectDB.EnumRecordsByParentID(garrAbilityRec.ID, (GarrAbilityEffectRec garrAbilityEffectRec) => {
					...
					if (sprite == null)
					{
						this.m_counteredMechanicName.text = string.Concat("ERR ", record.IconFileDataID);
					}
					else
					{
						this.m_counteredMechanicIcon.sprite = sprite;
					}
					this.m_counteredGarrMechanicTypeID = record.ID;
					return false;
				});
			}
			else
