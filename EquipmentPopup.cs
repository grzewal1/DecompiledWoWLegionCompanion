...
public class EquipmentPopup : MonoBehaviour
{
	...
	public void SetEquipment(MobileFollowerEquipment item, int garrFollowerID)
	{
		...
		for (int j = 0; j < (int)jamGarrisonFollower.AbilityID.Length; j++)
		{
			if ((StaticDB.garrAbilityDB.GetRecord(jamGarrisonFollower.AbilityID[j]).Flags & 1) != 0)
			{
				...
				gameObject.GetComponent<FollowerEquipmentReplacementSlot>().SetAbility(jamGarrisonFollower.AbilityID[j]);
			}
		}
