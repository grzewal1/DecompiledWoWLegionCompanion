...
public class LevelUpToast : MonoBehaviour
{
	...
	{
		Vector2 mGlintRT = this.m_glintRT.anchoredPosition;
		mGlintRT.x = 10f + 240f * val;
		this.m_glintRT.anchoredPosition = mGlintRT;
		float single = 0.5f;
		float single1 = 0.6f;
		if (val < single)
		{
			this.m_glintCanvasGroup.alpha = val / single;
		}
		else if (val >= single1)
		{
			this.m_glintCanvasGroup.alpha = 1f - (val - single1) / (1f - single1);
		}
		else
		{
	...
	private void Phase1Complete()
	{
		...
		iTween.ValueTo(base.gameObject, iTween.Hash(new object[] { "name", "Phase2", "delay", this.m_delayUntilPhase2Duration, "from", 1f, "to", 0f, "easeType", "easeOutCubic", "time", this.m_glowFadeOutDuration, "onupdate", "Phase2Update", "oncomplete", "Phase2Complete" }));
	}
	...
	{
		this.m_glowCanvasGroup.alpha = 0f;
		iTween.ValueTo(base.gameObject, iTween.Hash(new object[] { "name", "Phase3", "delay", this.m_delayUntilPhase3Duration, "from", 1f, "to", 0f, "easeType", "easeInCubic", "time", this.m_toastFadeOutDuration, "onupdate", "Phase3Update", "oncomplete", "Phase3Complete" }));
	}
	...
	{
		this.m_level.text = string.Concat(string.Empty, newLevel);
		Main.instance.m_UISound.Play_PlayerLevelUp();
		iTween.ValueTo(base.gameObject, iTween.Hash(new object[] { "name", "Phase1", "delay", 0f, "from", 0f, "to", 1f, "easeType", "easeInCubic", "time", this.m_initialFadeInDuration, "onupdate", "Phase1Update", "oncomplete", "Phase1Complete" }));
		iTween.ValueTo(base.gameObject, iTween.Hash(new object[] { "name", "GlintPhase1", "delay", this.m_glintInitialDelayDuration, "from", 0f, "to", 1f, "easeType", iTween.EaseType.easeInOutCubic, "time", this.m_glintDuration, "onupdate", "GlintPhase1Update", "oncomplete", "GlintPhase1Complete" }));
	}
}