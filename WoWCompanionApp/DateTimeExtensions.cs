...
namespace WoWCompanionApp
{
	public static class DateTimeExtensions
	{
		...
		public static string GetDurationString(this TimeSpan timeSpan, bool displayLargestUnitOnly = false)
		{
			...
			if (timeSpan.Days > 0)
			{
				stringBuilder.ConcatFormat<int, string>("{0} {1}", timeSpan.Days, (timeSpan.Days != 1 ? DateTimeExtensions.m_daysText : DateTimeExtensions.m_dayText));
				if (!displayLargestUnitOnly && timeSpan.Hours > 0)
				{
					stringBuilder.ConcatFormat<int, string>(" {0} {1}", timeSpan.Hours, DateTimeExtensions.m_hourText);
				}
			}
			else if (timeSpan.Hours > 0)
			{
				...
				{
					stringBuilder.ConcatFormat<int, string>(" {0} {1}", timeSpan.Minutes, DateTimeExtensions.m_minuteText);
				}
			}
			else if (timeSpan.Minutes > 0)
			{
				...
				{
					stringBuilder.ConcatFormat<int, string>(" {0} {1}", timeSpan.Seconds, DateTimeExtensions.m_secondText);
				}
			}
