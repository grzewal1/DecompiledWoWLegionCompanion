...
namespace WoWCompanionApp
{
	public class OptionsDialog : MonoBehaviour
	{
		...
		{
			bool flag;
			using (IEnumerator<WrapperWorldQuestBounty> enumerator = PersistentBountyData.bountyDictionary.Values.GetEnumerator())
			{
				while (enumerator.MoveNext())
				{
					if (enumerator.Current.QuestID != bountyQuestID)
					{
						continue;
					}
					flag = true;
		...
		private string GetQuestTitle(int questID)
		{
			...
			Debug.LogError(string.Concat("Invalid Quest ID ", questID));
			return string.Empty;
		}
		...
		public void SyncWithOptions()
		{
			...
			string str = SecurePlayerPrefs.GetString("EnableSFX", Main.uniqueIdentifier);
			if (str != null)
			{
				if (str == "true")
				{
			...
			bool mEnableNotifications = Main.instance.m_enableNotifications;
			string str1 = SecurePlayerPrefs.GetString("EnableNotifications", Main.uniqueIdentifier);
			if (str1 != null)
			{
				if (str1 == "true")
				{
			...
			if (Main.instance.m_enableNotifications != mEnableNotifications)
			{
				Main.instance.m_enableNotifications = mEnableNotifications;
			}
