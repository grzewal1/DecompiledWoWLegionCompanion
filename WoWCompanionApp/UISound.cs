...
namespace WoWCompanionApp
{
	public class UISound : MonoBehaviour
	{
		...
		public void PlayUISound(string soundName, float volumeScale = 1f, int maxInstances = 3)
		{
			...
			int num1 = 0;
			while (num1 < (int)this.m_oneShotAudioSources.Length)
			{
				if (!this.m_oneShotAudioSources[num1].isPlaying)
				{
					AudioClip audioClip = Resources.Load<AudioClip>(soundName);
					this.m_oneShotAudioSources[num1].PlayOneShot(audioClip, volumeScale);
					this.m_oneShotAudioSourceNames[num1] = soundName;
					break;
				}
				else
				{
