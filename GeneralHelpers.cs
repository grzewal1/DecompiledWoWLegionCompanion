...
public class GeneralHelpers : MonoBehaviour
{
	...
	public static Color GetQualityColor(int quality)
	{
		...
		Color result = new Color(0.34f, 0.49f, 1f, 1f);
		Color result2 = new Color(0.58f, 0f, 1f, 1f);
		Color result3 = new Color(1f, 0.56f, 0f, 1f);
		switch (quality)
		{
		case 1:
			return gray;
	...
	{
		return (int)DateTime.get_UtcNow().Subtract(new DateTime(1970, 1, 1)).get_TotalSeconds();
	}
			...
			else
			{
				StaticDB.garrAbilityEffectDB.EnumRecordsByParentID(garrAbilityRec.ID, delegate(GarrAbilityEffectRec garrAbilityEffectRec)
				{
					...
					case 1u:
					{
						...
						{
							MissionFollowerSlot missionFollowerSlot = array[j];
