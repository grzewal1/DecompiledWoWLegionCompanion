...
public class TalentTreeItemAbilityButton : MonoBehaviour
{
	...
	{
		long researchDurationSecs = (long)this.m_garrTalentRec.ResearchDurationSecs - (GarrisonStatus.CurrentTime() - (long)this.m_talent.StartTime);
		return researchDurationSecs;
	}

	public long GetRemainingRespecTime()
	{
		long respecDurationSecs = (long)this.m_garrTalentRec.RespecDurationSecs - (GarrisonStatus.CurrentTime() - (long)this.m_talent.StartTime);
		return respecDurationSecs;
	}
	...
	public bool IsOwned()
	{
		return (this.m_talent == null ? false : (this.m_talent.Flags & 1) != 0);
	}
	...
	{
		return (this.m_talent == null || this.IsOwned() ? false : this.m_talent.StartTime > 0);
	}

	public bool IsRespec()
	{
		return (this.m_talent == null ? false : (this.m_talent.Flags & 2) != 0);
	}
	...
	public void StartResearch()
	{
		if (this.m_canResearch)
		{
			...
			{
				GarrTalentID = this.m_garrTalentRec.ID
			};
	...
	private void Update()
	{
		...
		{
			this.m_researchProgressBG.gameObject.SetActive(true);
			float single = 1f - (!this.IsRespec() ? (float)this.GetRemainingResearchTime() / (float)this.m_garrTalentRec.ResearchDurationSecs : (float)this.GetRemainingRespecTime() / (float)this.m_garrTalentRec.RespecDurationSecs);
