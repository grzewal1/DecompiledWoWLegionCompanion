...
public class FollowerInventoryListItem : MonoBehaviour
{
	...
	private void Awake()
	{
		...
		{
			this.m_combatAllyUnavailableDarkener.get_gameObject().SetActive(false);
		}
		this.m_expandableAreaCanvasGroup.set_interactable(false);
		if (this.m_equipmentName != null)
		{
			this.m_equipmentName.set_font(GeneralHelpers.LoadStandardFont());
		}
		if (this.m_equipmentQuantity != null)
		{
			this.m_equipmentQuantity.set_font(GeneralHelpers.LoadStandardFont());
		}
		if (this.m_equipmentDescription != null)
		{
			this.m_equipmentDescription.set_font(GeneralHelpers.LoadStandardFont());
		}
		if (this.m_combatAllyName != null)
		{
			this.m_combatAllyName.set_font(GeneralHelpers.LoadStandardFont());
		}
		if (this.m_combatAllyStatus != null)
		{
			this.m_combatAllyStatus.set_font(GeneralHelpers.LoadStandardFont());
		}
		if (this.m_useItemButtonLabel != null)
		{
			this.m_useItemButtonLabel.set_font(GeneralHelpers.LoadStandardFont());
		}
	...
	private void SetCombatAllyAvailabilityStatus()
	{
		...
		if (flag5)
		{
			...
			this.m_combatAllyUnavailableDarkener.get_gameObject().SetActive(false);
			this.m_mainButton.set_enabled(true);
		...
		if (flag)
		{
			this.m_combatAllyStatus.set_color(Color.get_red());
			this.m_combatAllyStatus.set_text(FollowerInventoryListItem.m_inactiveString);
			this.m_combatAllyUnavailableDarkener.set_color(new Color(1f, 0.4f, 0.4f, 1f));
		}
		else if (flag2)
		{
			this.m_combatAllyStatus.set_text(FollowerInventoryListItem.m_fatiguedString);
			this.m_combatAllyUnavailableDarkener.set_color(new Color(0.4f, 0.4f, 1f, 1f));
		}
		else if (flag3)
		{
			this.m_combatAllyStatus.set_text(FollowerInventoryListItem.m_onMissionString);
			this.m_combatAllyUnavailableDarkener.set_color(new Color(0.4f, 0.4f, 1f, 1f));
		}
		else if (flag4)
		{
			this.m_combatAllyStatus.set_text(FollowerInventoryListItem.m_inBuildingString);
			this.m_combatAllyUnavailableDarkener.set_color(new Color(0.4f, 0.4f, 1f, 1f));
		}
	}
	...
	{
		this.m_expandableAreaLayoutElement.set_minHeight(val);
	}

	private void FinishExpandingCallback()
	{
		this.m_expandableAreaLayoutElement.set_minHeight((float)this.m_expandableAreaMaxHeight);
		this.m_expandableAreaCanvasGroup.set_interactable(true);
		this.m_isExpanded = true;
	}

	private void FinishContractingCallback()
	{
		this.m_expandableAreaLayoutElement.set_minHeight(0f);
		this.m_isExpanded = false;
	}

	private void UpdateExpandableAreaAlpha(float val)
	{
		this.m_expandableAreaCanvasGroup.set_alpha(val);
	}

	private void FinishFadingExpandableAreaOut()
	{
		this.m_expandableAreaCanvasGroup.set_alpha(0f);
	}

	private void FinishFadingExpandableAreaIn()
	{
		this.m_expandableAreaCanvasGroup.set_alpha(1f);
	}
	...
	public void HandleTapMainListItem()
	{
		...
		if (!this.m_isExpanded)
		{
			iTween.ValueTo(base.get_gameObject(), iTween.Hash(new object[]
			{
				...
				"Expand",
				"from",
				this.m_expandableAreaLayoutElement.get_minHeight(),
				"to",
			...
			iTween.ValueTo(base.get_gameObject(), iTween.Hash(new object[]
			{
				...
				"FadeIn",
				"from",
				this.m_expandableAreaCanvasGroup.get_alpha(),
				"to",
				1f,
				"easeType",
				"easeOutCubic",
	...
	{
		this.m_equipmentName.set_text(text);
	}
}
