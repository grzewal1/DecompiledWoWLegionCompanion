...
public class TalentTooltip : MonoBehaviour
{
	...
	public void SetTalent(TalentTreeItemAbilityButton abilityButton)
	{
		...
		Duration duration = new Duration((abilityButton.CanRespec() || flag || flag1 ? this.m_garrTalentRec.RespecDurationSecs : this.m_garrTalentRec.ResearchDurationSecs), false);
		...
		else
		{
			...
			{
				Duration duration1 = new Duration((!abilityButton.IsRespec() ? (int)this.m_abilityButton.GetRemainingResearchTime() : (int)this.m_abilityButton.GetRemainingRespecTime()), false);
				this.m_statusText.text = string.Concat(new string[] { "<color=#FFC600FF>", StaticDB.GetString("TIME_LEFT", null), "</color> <color=#ffffffff>", duration1.DurationString, "</color>" });
			}
